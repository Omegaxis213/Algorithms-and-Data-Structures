...
public class HLDNode {
	...
	public static void main(String[] args) throws Exception {
		BufferedReader in = new BufferedReader(new FileReader("HLDNode.in"));
		PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter("HLDNode.out")));
		String line;
		...
		for (int i = 0; i < queries; i++)
		{
			...
			if(type.equals("P"))
			{
				...
				chainSegtree.updateInterval(1,0,runs-1,chainPos[chainOne]+chains[chainOne].map.get(lcaNode),chainPos[chainOne]+Math.max(chains[chainOne].map.get(one),chains[chainOne].map.get(two)),1);
			}
			else
			{
				int res=0;
